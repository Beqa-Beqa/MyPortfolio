...
const Header = () => {
  ...
  const navLinks = useCallback((direction: "row" | "column" = "row", gap: number = 3) => {
    ...
    return (
      <Nav variant="underline" className={`fs-5 gap-${gap} ${direction === "column" && "flex-column"}`}>
        <Nav.Link onClick={() => clickAction("aboutme")}>About me</Nav.Link>
  ...
  return (
    <Stack
      ...
      className="text-center text-lg-start justify-content-between position-fixed top-0 start-0 w-100 px-xl-5 px-md-4 px-sm-2 px-1 py-3"
    >
      <div style={{filter: "blur(100px)"}} className="w-100 position-absolute top-0 start-0" />
      <div className="tracking-in-expand align-self-start">
        <h1 className="title display-5">Beka Aladashvili</h1>
      </div>
      {screenWidth <= 768 ?
        <>
          <RxHamburgerMenu onClick={() => setIsBurgerMenuOpen(true)} className="icon" />
          <div className={`burger-menu pt-2 ${isBurgerMenuOpen && "active"}`}>
              <MdClose onClick={() => setIsBurgerMenuOpen(false)} className="icon mt-2 me-3 position-absolute end-0" />
